...
import { generateProof, verifyProof, type Proof } from "./generateProof.js";

/**
 * Helper: Create mock leaves with predictable content for testing
 */
function createMockLeaves(count: number): string[] {
  return Array.from({ length: count }, (_, i) => `leaf_${i}`);
}
