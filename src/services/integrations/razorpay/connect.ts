...
export async function connectRazorpay(req: Request, res: Response) {
  ...
  if (!apiKeyId || !apiKeySecret) {
    return res
      .status(400)
      .json({ error: "apiKeyId and apiKeySecret are required" });
  }
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return res
        .status(400)
        .json({ error: "Invalid Razorpay credentials", details: text });
    }
  } catch (err: any) {
    return res
      .status(502)
      .json({ error: "Failed to reach Razorpay API", details: String(err) });
  }
  ...
  const safe = {
    ...record,
    meta: { apiKeyId: record.meta.apiKeyId, apiKeySecret: "*****" },
  };
