 ...
 */
export const MERKLE_MAX_LEAVES = (() => {
    const raw = process.env.MERKLE_MAX_LEAVES;
    if (!raw)
        return 1_048_576; // 2^20
    const parsed = parseInt(raw, 10);
    if (!Number.isInteger(parsed) || parsed <= 0 || parsed > 16_777_216) {
        throw new Error(`MERKLE_MAX_LEAVES must be a positive integer ≤ 16777216, got: "${raw}"`);
    }
    return parsed;
})();
/**
 ...
 */
export const MERKLE_WARN_LEAVES = Math.floor(MERKLE_MAX_LEAVES * 0.1);
// ─── Tree builder ─────────────────────────────────────────────────────────────
/**
...
export function buildTree(leaves) {
    ...
    if (leaves.length > MERKLE_MAX_LEAVES) {
        throw new RangeError(`Leaf count ${leaves.length} exceeds MERKLE_MAX_LEAVES (${MERKLE_MAX_LEAVES}). ` +
 ...
 */
export function getRoot(tree, _leafCount) {
