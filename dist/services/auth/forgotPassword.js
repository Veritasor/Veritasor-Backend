...
function resolveTokenTtlMinutes() {
    ...
    if (!Number.isFinite(parsed) || parsed <= 0 || parsed > 60) {
        process.stderr.write(`[forgotPassword] RESET_TOKEN_TTL_MINUTES="${raw}" is invalid; using default ${DEFAULT} min\n`);
        return DEFAULT;
    }
    return parsed;
}
 ...
 */
export async function forgotPassword(request, logger = () => { }) {
    const { email } = request;
