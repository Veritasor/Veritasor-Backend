...
// ── Helpers ───────────────────────────────────────────────────────────────────
function resolveMinPasswordLength() {
    const DEFAULT = 8;
    ...
    if (!Number.isFinite(parsed) || parsed < 8) {
        process.stderr.write(`[resetPassword] RESET_MIN_PASSWORD_LENGTH="${raw}" is invalid or below minimum 8; using ${DEFAULT}\n`);
        return DEFAULT;
    }
    return parsed;
}
 ...
 */
export async function resetPassword(request, logger = () => { }) {
    ...
    if (newPassword.length < minLength) {
        throw new AppError(`Password must be at least ${minLength} characters`, 400, 'VALIDATION_ERROR');
    }
