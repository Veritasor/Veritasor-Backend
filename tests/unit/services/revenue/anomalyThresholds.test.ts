...
describe("threshold configuration snapshot validation", () => {
  it("defaults match expected snapshot values", () => {
    ...
    expect(THRESHOLD_SNAPSHOT.DEFAULT_ROLLING_WINDOW).toBe(3);
  });

  it("keeps the fixture contract metadata aligned with the reviewed snapshot", () => {
    expect(ANOMALY_FIXTURE_CONTRACT.version).toBe("2026-08-29");
    expect(ANOMALY_FIXTURE_CONTRACT.thresholds.DROP_THRESHOLD).toBe(0.4);
    expect(ANOMALY_FIXTURE_CONTRACT.thresholds.SPIKE_THRESHOLD).toBeGreaterThan(
      ANOMALY_FIXTURE_CONTRACT.thresholds.DROP_THRESHOLD,
    );
  });
